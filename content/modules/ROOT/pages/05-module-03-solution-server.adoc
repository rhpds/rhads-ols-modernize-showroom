...
+
There are two different browser tabs.  One is the DevSpaces tab/window that lists all the active DevSpaces workspaces.  The other one is the the actual opened workspace, which is the VSCode IDE hosted in a browser window/tab.
+
...
. Switch to the __Module__ 3 Git branch
+
...
* Enter the following into to the terminal prompt and hit Enter:
+
[source,bash]
----
...
* Enable/Turn on __Skip SSL certificate verification__
