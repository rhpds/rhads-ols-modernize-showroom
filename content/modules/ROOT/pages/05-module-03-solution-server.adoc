...
+
* Notice __Hub Configuration__ is not yet configured and __Configure GenAI__ may not be configured yet also
...
* As you scroll through the __OrderService.java__ file notice **Red** suggested removals and **Green** suggested fixes generated by Developer Lightspeed through the LLM with rules prompting. Recall that the static rules run during the **Analysis** and are used to focus the LLM on defined categories of code issues in migration efforts.
+
...
+
* Notice the differing approach, switching to using environment variables as the primary way to set the configuration in the running application.
* Delete the original **green** highlighted code snippet. (The **red** areas marked for deletion will disappear in the next step)
+
...
Going forward Solution Server will pass hints into the LLM for what future fixes should take into account.  Thus both static rules pinpointing areas of concern and enhanced hints from Solution Server will be incorporated into LLM code fixes.
====
...
__Normally the LLM would not be able to create these fixes by itself since the context and policy is specific to ACME.  Solution Server + Developer LIghtspeed for MTA provide this unique AI capability__

=== Steps

. Fix another occurance of the learned policy issue and resolutin approach:
+
...
You can can now accept changes and close the file and Resolution Pane 
